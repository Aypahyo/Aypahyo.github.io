...
I plan on programming TicTacToe and deleting it afterwards from my harddrive, there will be a copy on this server - so I wont use a repository. For testing I will create a seperate project that connects to NUnit and gets access to internals of TicTacToe. I also  want to see <em>FXCop</em> from Microsoft in action. <em>FXCop</em> is a styleguard and will help me to conform to the C# default coding style. On the framework side of things I can currently only think about <em>WPF</em> for the GUI, the alternative would be XNA and i think that may be quite some overkill for this project.
